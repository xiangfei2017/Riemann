...
.. list-table:: 形状和维度操作函数
  ...
  * - ``hstack``
    ...
    - ``rm.hstack([x, y])``
  * - ``split``
    - 沿指定维度将张量分割成多个块（按块大小分割）
    - ``rm.split(x, 2, dim=0)`` 或 ``rm.split(x, [2, 3], dim=0)``
  * - ``tensor_split``
  ...
  * - ``hsplit``
    ...
    - ``rm.hsplit(x, 2)`` 或 ``rm.hsplit(x, [1, 3])``
  * - ``dsplit``
    - 深度分割张量（沿第2维），将3D+张量分割为多个子张量
    - ``rm.dsplit(x, 3)`` 或 ``rm.dsplit(x, [2, 4])``
  * - ``dstack``
    - 深度堆叠张量，沿第2维堆叠（1D/2D张量会先reshape）
    - ``rm.dstack([x, y])``
