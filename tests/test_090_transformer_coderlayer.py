...
def sync_encoder_params(rm_layer, torch_layer):
    """同步Encoder Layer的参数"""
    # 自注意力
...
def sync_decoder_params(rm_layer, torch_layer):
    """同步Decoder Layer的参数"""
    # 自注意力
    rm_layer.self_attn.in_proj_weight.data[:] = rm.tensor(torch_layer.self_attn.in_proj_weight.detach().numpy())
    if torch_layer.self_attn.in_proj_bias is not None:
...
class TestTransformerCoderLayer(unittest.TestCase):
    def setUp(self):
        # 设置随机种子以确保结果可重复
        np.random.seed(42)
        if TORCH_AVAILABLE:
    ...
    def _run_encoder_test_case(self, case_name, **kwargs):
        ...
        use_src_mask = kwargs.get('use_src_mask', False)
        use_src_key_padding_mask = kwargs.get('use_src_key_padding_mask', False)
        ...
        try:
            ...
            if use_src_mask:
                ...
                rm_src_mask = rm.tensor(mask_data)
                torch_src_mask = torch.tensor(mask_data)
            
            rm_src_key_padding_mask = None
            torch_src_key_padding_mask = None
            if use_src_key_padding_mask:
                mask_shape = (batch_size, seq_len)
                mask_data = np.random.choice([False, True], size=mask_shape, p=[0.8, 0.2])
                rm_src_key_padding_mask = rm.tensor(mask_data)
                torch_src_key_padding_mask = torch.tensor(mask_data)
            
            # 创建模块（注意dropout设置为0.0，避免随机性）
            rm_encoder = rm_nn.TransformerEncoderLayer(
                d_model=d_model, nhead=nhead, dim_feedforward=dim_feedforward,
                dropout=0.0, activation=activation,
                norm_first=norm_first, batch_first=batch_first
            )
            
            torch_encoder = torch_nn.TransformerEncoderLayer(
                d_model=d_model, nhead=nhead, dim_feedforward=dim_feedforward,
                dropout=0.0, activation=activation,
                norm_first=norm_first, batch_first=batch_first
            )
            ...
            # 同步参数
            sync_encoder_params(rm_encoder, torch_encoder)
            
            # 前向传播
            rm_output = rm_encoder(
                rm_src, src_mask=rm_src_mask,
                src_key_padding_mask=rm_src_key_padding_mask,
                is_causal=is_causal
            )
            
            # PyTorch: 处理is_causal时需要明确的mask
            if is_causal and torch_src_mask is None:
                np_causal_mask = np.triu(np.ones((seq_len, seq_len), dtype=np.float32), k=1) * -1e4
                torch_src_mask = torch.tensor(np_causal_mask)
                rm_src_mask_actual = rm.tensor(np_causal_mask)
                rm_output = rm_encoder(
                    rm_src, src_mask=rm_src_mask_actual,
                    src_key_padding_mask=rm_src_key_padding_mask,
                    is_causal=False
                )
            
            torch_output = torch_encoder(
                torch_src, src_mask=torch_src_mask,
                src_key_padding_mask=torch_src_key_padding_mask,
                is_causal=is_causal if torch_src_mask is not None else False
            )
            ...
            passed_param_grad = passed_param_grad and compare_values(rm_encoder.self_attn.in_proj_weight.grad, torch_encoder.self_attn.in_proj_weight.grad)
            passed_param_grad = passed_param_grad and compare_values(rm_encoder.self_attn.out_proj.weight.grad, torch_encoder.self_attn.out_proj.weight.grad)
            passed_param_grad = passed_param_grad and compare_values(rm_encoder.linear1.weight.grad, torch_encoder.linear1.weight.grad)
            passed_param_grad = passed_param_grad and compare_values(rm_encoder.linear2.weight.grad, torch_encoder.linear2.weight.grad)
    ...
    def _run_decoder_test_case(self, case_name, **kwargs):
        ...
        try:
            ...
            # 创建模块（注意dropout设置为0.0，避免随机性）
            rm_decoder = rm_nn.TransformerDecoderLayer(
                d_model=d_model, nhead=nhead, dim_feedforward=dim_feedforward,
                dropout=0.0, activation=activation,
                norm_first=norm_first, batch_first=batch_first
            )
            
            torch_decoder = torch_nn.TransformerDecoderLayer(
                d_model=d_model, nhead=nhead, dim_feedforward=dim_feedforward,
                dropout=0.0, activation=activation,
                norm_first=norm_first, batch_first=batch_first
            )
            ...
            # 同步参数
            sync_decoder_params(rm_decoder, torch_decoder)
            ...
            # 比较参数梯度
            passed_param_grad = True
            passed_param_grad = passed_param_grad and compare_values(rm_decoder.self_attn.in_proj_weight.grad, torch_decoder.self_attn.in_proj_weight.grad)
            passed_param_grad = passed_param_grad and compare_values(rm_decoder.multihead_attn.in_proj_weight.grad, torch_decoder.multihead_attn.in_proj_weight.grad)
            passed_param_grad = passed_param_grad and compare_values(rm_decoder.linear1.weight.grad, torch_decoder.linear1.weight.grad)
            passed_param_grad = passed_param_grad and compare_values(rm_decoder.linear2.weight.grad, torch_decoder.linear2.weight.grad)
    ...
    def test_encoder_norm_first(self):
        """Encoder Layer - norm_first=True"""
        self._run_encoder_test_case("norm_first=True", norm_first=True)
    
    def test_encoder_batch_first(self):
        """Encoder Layer - batch_first=True"""
        self._run_encoder_test_case("batch_first=True", batch_first=True)
    
    def test_encoder_gelu(self):
        """Encoder Layer - activation='gelu'"""
        self._run_encoder_test_case("activation=gelu", activation='gelu')
    
    def test_encoder_large_dim(self):
        """Encoder Layer - 大维度"""
        self._run_encoder_test_case("大维度", d_model=256, nhead=8, dim_feedforward=1024)
    
    def test_encoder_src_mask(self):
        """Encoder Layer - 带src_mask"""
        self._run_encoder_test_case("带src_mask", use_src_mask=True)
    
    def test_encoder_src_key_padding_mask(self):
        """Encoder Layer - 带src_key_padding_mask"""
        self._run_encoder_test_case("带src_key_padding_mask", use_src_key_padding_mask=True)
    
    def test_encoder_is_causal(self):
        """Encoder Layer - is_causal=True"""
        self._run_encoder_test_case("is_causal=True", is_causal=True)
    
    # ==============================
    # TransformerDecoderLayer 测试
    # ==============================
...
if __name__ == '__main__':
    ...
    clear_screen()
    print("=" * 70)
    print("Riemann 与 PyTorch Transformer层一致性测试")
    print("=" * 70)
