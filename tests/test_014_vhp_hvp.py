...
class TestVhpHvpFunctions(unittest.TestCase):
    ...
    def test_strict_parameter(self):
        ...
        try:
            ...
            # 所有测试都应该通过
            passed = hvp_strict_false_passed and hvp_strict_true_passed and vhp_strict_true_passed
            
            time_taken = time.time() - start_time
            
            if IS_RUNNING_AS_SCRIPT:
                stats.add_result("HVP strict=False", hvp_strict_false_passed)
                stats.add_result("HVP strict=True", hvp_strict_true_passed)
                stats.add_result("VHP strict=False", vhp_strict_false_passed)
                stats.add_result("VHP strict=True", vhp_strict_true_passed)
                
                print(f"测试用例: HVP strict=False - {Colors.OKGREEN if hvp_strict_false_passed else Colors.FAIL}{'通过' if hvp_strict_false_passed else '失败'}{Colors.ENDC}")
                print(f"测试用例: HVP strict=True - {Colors.OKGREEN if hvp_strict_true_passed else Colors.FAIL}{'通过' if hvp_strict_true_passed else '失败'}{Colors.ENDC}")
                print(f"测试用例: VHP strict=False - {Colors.OKGREEN if vhp_strict_false_passed else Colors.FAIL}{'通过' if vhp_strict_false_passed else '失败'}{Colors.ENDC}")
                print(f"测试用例: VHP strict=True - {Colors.OKGREEN if vhp_strict_true_passed else Colors.FAIL}{'通过' if vhp_strict_true_passed else '失败'}{Colors.ENDC}")
            
            # 断言确保测试通过
            self.assertTrue(passed, "strict参数测试失败")
            
        except Exception as e:
            time_taken = time.time() - start_time
            if IS_RUNNING_AS_SCRIPT:
                stats.add_result("strict参数测试", False, [str(e)])
                print(f"测试用例: strict参数测试 - {Colors.FAIL}错误{Colors.ENDC} ({time_taken:.4f}秒) - {str(e)}")
            raise
