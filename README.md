...
Riemann是一个开源的轻量级自动微分库，专为自动微分和神经网络相关的学习、教育和研究目的而设计。该库专注于提供高效、灵活的自动微分功能，支持深度学习和科学计算领域的梯度计算需求，同时保持代码的可读性和可理解性，便于学习者深入理解自动微分原理和实现。

## 主要功能

- **自动微分**：支持实数和复数的标量、向量、张量的前向计算和反向自动微分
- **梯度计算**：支持反向传播算法计算梯度，提供`grad`和`backward`函数用于高效梯度计算，支持标量、向量、矩阵、多维张量计算中的反向梯度跟踪
- **张量操作**：提供丰富的张量操作功能，包括：加减乘除、初等函数、索引操作、形状操作、维度扩缩、堆叠/分割
- **高阶导数**：支持雅可比矩阵(Jacobian)和海森矩阵(Hessian)计算，以及JVP(雅可比-向量乘积)、VJP(向量-雅可比乘积)、HVP(海森-向量乘积)和VHP(向量-海森乘积)
...
- **自动微分机制**：`requires_grad`、计算图构建和反向传播机制与PyTorch相似
...
### 1. 张量操作
- 支持基本的数学运算（加法、减法、乘法、除法、幂、指数函数、对数函数、三角函数等）
- 提供张量创建函数（zeros, ones, random等）
...
- **hessian.py**: 海森矩阵计算示例
- **jacobian.py**: 雅可比矩阵计算示例
- **nn_MNIST_CE_SGD.py**: 基于MNIST的手写数字识别神经网络示例
- **cnn_CIFAR10_CE_SGD.py**: 基于CIFAR10的卷积神经网络示例
